...
def find_recall_type(label_dict, topk_idx):
    if topk_idx[0] in label_dict.values().tolist():
        res ="recall@1"
    elif any(topk_idx[:5].tolist()) in label_dict.values().tolist():
        res = "recall@5"
    elif any(topk_idx[:10].tolist()) in label_dict.values().tolist():
        res = "recall@10"
    else: 
        res = "negative sample"
    return res
...
def compute_i2t_sim_matrix_adapt_itm(model, data_loader, optimizer, tta_cfg, epoch, **kwargs):
    ...
    start_time = time.time()
    model.train()
    for i, sims_i2t in tqdm(enumerate(sims_matrix_i2t[start:end]), total=sims_matrix_i2t.size(0)): # 遍历每个image与25010个text的sim_matrix
