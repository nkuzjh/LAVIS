...
class TTA_T2I_Dataset(Dataset):
    ...
    def __getitem__(self, index): #TODO
        ...
        idxs = self.sims_idxs[index]
        ### vis feature
        image_inputs = self.vit_feats[index].unsqueeze(1).repeat(1, self.tta_cfg.k_tta, 1, 1)# vit_feats[i].shape=1,677,1408; image_inputs.shape=1,k_tta,677,1408
        image_inputs = image_inputs.reshape(-1, image_inputs.size(2), image_inputs.size(3)) # 1*k_tta,677,1408
        ### txt feature
        text_ids_inputs = self.text_ids[idxs]
        text_ids_inputs = text_ids_inputs.reshape(-1, text_ids_inputs.size(-1)) # 1*k_tta,35
        text_atts_inputs = self.text_atts[idxs]
    ...
    def collater(batch):
        ...
        # 特殊处理 idx（int），保留为 list
        collated['index'] = [item['index'] for item in batch]

        # 处理 tta_coeffi：可能是标量 tensor 或 shape [1]
        collated['tta_coeffi'] = torch.stack([item['tta_coeffi'].flatten() for item in batch], dim=0)

        # 其他张量字段使用 default_collate
        for key in batch[0]:
            if key in ['recall_type', 'idx', 'tta_coeffi']:
                continue  # 已经处理过了
